...
class ChatGroq:
    ...
    def __init__(self, model, temperature=0.0, max_retries=2):
        self.model = model
...
def implement_llama_4_model():
    ...
    """
    chat_groq = ChatGroq(model="llama-4-8b-instant", temperature=0.0)
    return chat_groq
...
def implement_llama_3_3_model():
    ...
    """
    chat_groq = ChatGroq("llama-3.3-70b-versatile", temperature=0.3)
    return chat_groq
...
def implement_query_model(model, prompt):
    ...
    """
    # Convert the prompt string to a list of messages as expected by ChatGroq.invoke()
    messages = [prompt]
...
def implement_compare_models(prompt):
    ...
    # Query both models
    llama4_response = implement_query_model(llama4, prompt)
    llama33_response = implement_query_model(llama33, prompt)
    
    # Return dictionary with both responses
    return {
        "Llama 4": llama4_response,
        "Llama 3.3": llama33_response
    }
